...
REVIEWS_WEBHOOK_URL = os.getenv("REVIEWS_WEBHOOK_URL", SHEETS_WEBHOOK_URL)  # тот же или отдельный
...
# --- Автоматическая рассылка «Накануне выпечки» ---
# Рассылка отправляется вечером накануне дня выпечки (Пн 19:00 → выпечка Вт, и т.д.)
# Время настраивается через env: PRE_BAKE_HOUR (по умолчанию 19 MSK).
# BAKE_WEEKDAYS задаёт дни выпечки (2=вт, 4=чт, 6=сб), рассылка уходит за день до них.
PRE_BAKE_HOUR = int(os.getenv("PRE_BAKE_HOUR", "19"))
PRE_BAKE_PROMO = os.getenv("PRE_BAKE_PROMO", "ЗАВТРА")
PRE_BAKE_DISCOUNT = os.getenv("PRE_BAKE_DISCOUNT", "−15%")
MSK_OFFSET = timezone(timedelta(hours=3))  # UTC+3, Москва

# --- Напоминание о брошенной корзине ---
# Через сколько часов после последнего изменения корзины отправлять напоминание.
ABANDONED_CART_HOURS = float(os.getenv("ABANDONED_CART_HOURS", "2"))
# Повторное напоминание (0 = не отправлять второе). По умолчанию через 24ч.
ABANDONED_CART_REPEAT_HOURS = float(os.getenv("ABANDONED_CART_REPEAT_HOURS", "24"))

# ================= КАТАЛОГ =================
PRODUCTS = [
    {"id": "wheat", "name": "Тартин", "weight": 650, "price": 380,
     "desc": "Классика: хрустящая корочка, пористый мякиш, лёгкая кислинка. 650 г."},
    {"id": "rye", "name": "Заварной ржано-пшеничный", "weight": 500, "price": 425,
     "desc": "Тёмный, плотный, солодовый вкус. Заварная технология. 500 г."},
    {"id": "seeds", "name": "Бородинский", "weight": 400, "price": 340,
     "desc": "С кориандром и мёдом. Долгая ферментация. 400 г."},
    {"id": "wholegrain", "name": "Тартин сырный", "weight": 650, "price": 468,
     "desc": "Тартин с кусочками Гауды/Грюйера внутри и сырной корочкой. 650 г."},
]

def load_products_from_sheets():
    """Загружает товары из Google Sheets (лист 'Товары'). Заменяет PRODUCTS."""
    global PRODUCTS
    url = REVIEWS_WEBHOOK_URL or SHEETS_WEBHOOK_URL
    if not url:
        return
    try:
        resp = requests.get(url, params={"action": "products"}, timeout=15)
        data = resp.json()
        ...
        print("Не удалось загрузить товары из Sheets (используются дефолтные):", e)
...
def push_review_to_sheets(name, score, text, sku, order_id, channel="telegram"):
    """Отправляет отзыв в Google Sheets через Apps Script."""
    url = REVIEWS_WEBHOOK_URL
    if not url:
        return
    try:
        requests.post(url, json={
            ...
            "orderId": order_id,
            "channel": channel,
            "status": "approved" if score >= 4 else "pending",
        }, timeout=15)
    except Exception as e:
        print("Не удалось записать отзыв в Google Sheets:", e)
...
async def start_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Deep link параметры из Instagram Stories, Reels, taplink
    args = context.args
    param = args[0].lower() if args else ""

    if param == "order" or param == "menu":
        # Instagram Stories → «Заказать» → сразу в каталог
        await update.message.reply_text(
            "🍞 Привет! Выберите хлеб — закажем на ближайшую выпечку (вт/чт/сб):",
        )
        # Переиспользуем логику menu_cmd
        session = get_session(chat_id)
        if not session["phone"]:
            session["stage"] = "linking_phone"
            await update.message.reply_text("Укажите номер телефона для оформления заказа:")
            return
        items = get_cart(session["phone"])["items"]
        await update.message.reply_text("Выберите хлеб:", reply_markup=catalog_keyboard(items))
        return

    if param == "checkout":
        # Из каталога сайта → оформить заказ в боте
        session = get_session(chat_id)
        if not session["phone"]:
            session["stage"] = "linking_phone"
            await update.message.reply_text("Укажите номер телефона чтобы загрузить корзину с сайта:")
            return
        items = get_cart(session["phone"])["items"]
        if items:
            await update.message.reply_text(
                f"🛒 Ваша корзина с сайта:\n\n{cart_text(items)}\n\n"
                f"💰 Итого: {cart_total(items)} ₽\n\nОформить заказ?",
                reply_markup=catalog_keyboard(items)
            )
        else:
            await update.message.reply_text(
                "Корзина пуста. Выберите хлеб:", reply_markup=catalog_keyboard({})
            )
        return

    if param == "nobroadcast":
        # Из рассылки → отписаться
        if chat_id not in broadcast_optout:
            broadcast_optout.append(chat_id)
            save_broadcast_optout()
        await update.message.reply_text("🔕 Вы отписаны от рассылки. Подписаться снова: /yesbroadcast")
        return

    # Стандартное приветствие
    await update.message.reply_text(
        "Привет! Это пекарня Масса Матере 🍞\n\n"
        "Хлеб на закваске ручной работы. Выпечка вт/чт/сб.\n\n"
        "Команды:\n"
        "/menu — каталог и заказ\n"
        "/cart — корзина\n"
        "/orders — мои заказы\n"
...
# ================= РАССЫЛКА (расширенная) =================
# Черновик рассылки: {admin_chat_id: {text, photo_id, button_text, button_url, segment, schedule_at}}
# Живёт только в памяти — не переживает рестарт (осознанно, см. комментарий выше).
pending_broadcasts = {}

# Отписка от рассылки — chat_id клиентов, которые выбрали /nobroadcast.
# Переживает рестарт, хранится в файле.
broadcast_optout = load_json("broadcast_optout.json", [])
def save_broadcast_optout(): save_json("broadcast_optout.json", broadcast_optout)

def get_broadcast_recipients(segment=None):
    """Возвращает список уникальных chat_id, исключая отписавшихся.
    segment — опциональный фильтр по product_id (например 'rye')."""
    all_ids = list(set(links.values()))
    # Исключаем отписавшихся
    active = [cid for cid in all_ids if cid not in broadcast_optout]
    if not segment:
        return active
    # Фильтр по продукту: оставляем только тех, кто когда-либо заказывал этот продукт
    segment_ids = set()
    for o in orders:
        if any(i.get("name", "").lower().find(segment.lower()) >= 0 or
               o.get("id", "").lower() == segment.lower()
               for i in o.get("items", [])):
            cid = o.get("chatId") or get_chat_id_for_phone(o.get("phone"))
            if cid and cid in active:
                segment_ids.add(cid)
    return list(segment_ids)

async def do_broadcast(bot, admin_chat_id, bc):
    """Выполняет рассылку по черновику bc. Возвращает (sent, failed)."""
    recipients = get_broadcast_recipients(bc.get("segment"))
    sent, failed = 0, 0
        ...
        try:
            if bc.get("photo_id"):
                ...
                if bc.get("button_text") and bc.get("button_url"):
                    markup = InlineKeyboardMarkup([[InlineKeyboardButton(bc["button_text"], url=bc["button_url"])]])
                else:
                    markup = None
                await bot.send_photo(cid, photo=bc["photo_id"], caption=bc.get("text", ""), reply_markup=markup)
            elif bc.get("button_text") and bc.get("button_url"):
                # Текст + кнопка-ссылка
                markup = InlineKeyboardMarkup([[InlineKeyboardButton(bc["button_text"], url=bc["button_url"])]])
                await bot.send_message(cid, bc["text"], reply_markup=markup)
            else:
                # Чистый текст
                await bot.send_message(cid, bc["text"])
            sent += 1
        except Exception as e:
            failed += 1
            print(f"Broadcast: не удалось отправить {cid}:", e)
        await asyncio.sleep(0.05)  # ~20 сообщений/сек — запас под лимит Telegram (30/сек)
    return sent, failed

# Отложенные рассылки: [{bc, admin_chat_id, schedule_at}]
scheduled_broadcasts = []

def check_scheduled_broadcasts_sync():
    """Вызывается из фонового планировщика (синхронный контекст). Проверяет и запускает
    рассылки, у которых наступило время."""
    now = datetime.now(timezone.utc)
    due = [sb for sb in scheduled_broadcasts if datetime.fromisoformat(sb["schedule_at"]) <= now]
    for sb in due:
        scheduled_broadcasts.remove(sb)
        bc = sb["bc"]
        recipients = get_broadcast_recipients(bc.get("segment"))
        sent, failed = 0, 0
        for cid in recipients:
            try:
                if bc.get("photo_id"):
                    markup = None
                    if bc.get("button_text") and bc.get("button_url"):
                        # Для синхронного контекста используем прямой HTTP-вызов
                        pass  # фото + кнопка через синхронный API сложнее — упрощаем до текста
                    send_telegram_message(cid, bc.get("text", "(фото-рассылка — откройте бота)"))
                else:
                    text = bc["text"]
            ...
            time.sleep(0.05)
        if ADMIN_CHAT_ID:
            send_telegram_message(ADMIN_CHAT_ID, f"⏰ Отложенная рассылка выполнена: доставлено {sent}, не удалось {failed}.")
...
async def broadcast_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Парсим флаги: --segment=rye --button="Заказать|https://t.me/bot" --at=2026-07-20T10:00
    import shlex
    text_parts = []
    segment = None
    button_text = None
    ...
    for token in raw.split(" "):
        if token.startswith("--segment="):
            segment = token.split("=", 1)[1]
        elif token.startswith("--button="):
            btn_raw = token.split("=", 1)[1].strip('"').strip("'")
            if "|" in btn_raw:
                button_text, button_url = btn_raw.split("|", 1)
        elif token.startswith("--at="):
            schedule_at = token.split("=", 1)[1]
        else:
    ...
    if not text and not photo:
        help_text = (
            ...
            "Базовая: /broadcast Текст сообщения\n\n"
            "С фото: отправьте фото с подписью, начинающейся с /broadcast\n\n"
            "Флаги (опциональные, через пробел после текста):\n"
            "  --segment=rye — только клиентам, заказывавшим ржаной хлеб\n"
            '  --button="Заказать|https://t.me/bot" — добавить кнопку-ссылку\n'
            "  --at=2026-07-20T10:00 — отложить отправку до указанного времени (UTC)\n\n"
            "Примеры:\n"
            "/broadcast Завтра выпечка! Заказы до 20:00\n"
            '/broadcast Новинка! --button="Попробовать|https://t.me/Sourdough_Bakery_Bot"\n'
            "/broadcast Скидка на ржаной! --segment=rye\n"
            "/broadcast Утренняя рассылка --at=2026-07-20T06:00"
        )
    ...
    pending_broadcasts[chat_id] = bc

    recipients = get_broadcast_recipients(segment)
    preview = text if len(text) <= 500 else text[:500] + "…"

    info_parts = [f"📢 Предпросмотр рассылки ({len(recipients)} получател{'ей' if len(recipients) != 1 else 'ь'}):"]
    if segment:
        info_parts.append(f"🎯 Сегмент: заказывавшие «{segment}»")
    if photo:
        info_parts.append("📷 С фотографией")
    if button_text:
        info_parts.append(f"🔗 Кнопка: [{button_text}]({button_url})")
    if schedule_at:
        info_parts.append(f"⏰ Отложено до: {schedule_at} UTC")
    info_parts.append(f"\n{preview}")

    await update.message.reply_text(
        "\n".join(info_parts),
        reply_markup=InlineKeyboardMarkup([[
            InlineKeyboardButton("✅ Отправить" + (" по расписанию" if schedule_at else " всем"),
                                 callback_data="broadcast_confirm"),
            InlineKeyboardButton("✖ Отмена", callback_data="broadcast_cancel"),
        ]]),
    )

async def nobroadcast_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Клиент отписывается от рассылок."""
    chat_id = update.effective_chat.id
    if chat_id not in broadcast_optout:
        broadcast_optout.append(chat_id)
        save_broadcast_optout()
        await update.message.reply_text("Вы отписались от рассылок. Если передумаете — /yesbroadcast")
    else:
        await update.message.reply_text("Вы уже отписаны от рассылок. Чтобы подписаться обратно — /yesbroadcast")

async def yesbroadcast_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Клиент подписывается обратно на рассылки."""
    chat_id = update.effective_chat.id
    if chat_id in broadcast_optout:
        broadcast_optout.remove(chat_id)
        save_broadcast_optout()
        await update.message.reply_text("Вы снова подписаны на рассылки пекарни 🍞")
    else:
        await update.message.reply_text("Вы и так подписаны на рассылки.")
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if data == "confirm":
            ...
            # Сбрасываем трекинг брошенной корзины
            if session["phone"] in abandoned_cart_tracking:
                del abandoned_cart_tracking[session["phone"]]
                save_abandoned_cart_tracking()
        ...
        if data.startswith("review:"):
            ...
            # Ищем SKU заказа для привязки отзыва к товару
            order_sku = ""
            for o in orders:
                if o.get("id") == order_id:
                    items = o.get("items", [])
                    if len(items) == 1:
                        order_sku = items[0].get("id", "")
                    break

            # Если 4-5★ — просим текст отзыва (опционально)
            if score_int >= 4:
                session = get_session(chat_id)
                session["stage"] = "awaiting_review_text"
                session["review_data"] = {
                    "orderId": order_id,
                    "score": score_int,
                    "sku": order_sku,
                ...
                await context.bot.send_message(
                    chat_id,
                    "Спасибо за оценку! 🙏\n\n"
                    "Напишите пару слов о хлебе — ваш отзыв появится на сайте.\n"
                    "Или отправьте /skip, чтобы пропустить."
                )
            else:
                # 1-3★: сохраняем без текста, не публикуем (pending)
                user = query.from_user
                name = user.full_name or user.username or "Покупатель"
                await asyncio.to_thread(
                    push_review_to_sheets, name, score_int, "", order_sku, order_id
                )
                await context.bot.send_message(
                    chat_id,
                    "Спасибо за обратную связь! 🙏 Мы учтём ваши замечания."
                )
            await query.answer()
            return
        ...
        if data == "broadcast_confirm":
            if not is_admin(chat_id):
                await query.answer("Только для администратора")
                return
            bc = pending_broadcasts.pop(chat_id, None)
            if not bc:
            ...
            # Отложенная рассылка
            if bc.get("schedule_at"):
                scheduled_broadcasts.append({"bc": bc, "admin_chat_id": chat_id, "schedule_at": bc["schedule_at"]})
                await context.bot.send_message(chat_id, f"⏰ Рассылка запланирована на {bc['schedule_at']} UTC. Она выполнится автоматически.")
                await query.answer()
                return
            ...
            sent, failed = await do_broadcast(context.bot, chat_id, bc)
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ─── Текст отзыва после оценки ───
    if session["stage"] == "awaiting_review_text":
        rd = session.get("review_data", {})
        user = update.effective_user
        name = user.full_name or user.username or "Покупатель"
        review_text = "" if text.strip().startswith("/skip") else text.strip()

        await asyncio.to_thread(
            push_review_to_sheets,
            name, rd.get("score", 5), review_text,
            rd.get("sku", ""), rd.get("orderId", ""),
        )
        session["stage"] = "idle"
        session.pop("review_data", None)

        if review_text:
            await update.message.reply_text("Отзыв опубликован на сайте! Спасибо 🍞")
            if ADMIN_CHAT_ID:
                await context.bot.send_message(
                    ADMIN_CHAT_ID,
                    f"💬 Отзыв от {name} по заказу №{rd.get('orderId','?')}:\n"
                    f"{'⭐' * rd.get('score', 5)}\n{review_text}"
                )
        else:
            await update.message.reply_text("Оценка сохранена. Спасибо! 🙏")
        return
...
# ================= РАССЫЛКА «НАКАНУНЕ ВЫПЕЧКИ» (автоматическая) =================
# Хранит дату последней отправки, чтобы не дублировать при рестарте в тот же день.
pre_bake_last_sent = load_json("pre_bake_last_sent.json", {"date": ""})
def save_pre_bake_last_sent():
    save_json("pre_bake_last_sent.json", pre_bake_last_sent)

BAKE_DAY_NAMES = {0: "понедельник", 1: "вторник", 2: "среду", 3: "четверг",
                  4: "пятницу", 5: "субботу", 6: "воскресенье"}

def build_pre_bake_message():
    """Генерирует текст автоматической рассылки «Завтра выпечка!»."""
    now_msk = datetime.now(MSK_OFFSET)
    tomorrow_wd = (now_msk.weekday() + 1) % 7
    bake_day = BAKE_DAY_NAMES.get(tomorrow_wd, "завтра")

    menu_lines = []
    for p in PRODUCTS:
        if not is_out_of_stock(p["id"]):
            menu_lines.append(f"  🍞 {p['name']} — {p['price']} ₽ / {p['weight']}г")
    menu = "\n".join(menu_lines) if menu_lines else "  (уточняйте наличие в боте)"

    return (
        f"🔔 Завтра выпечка!\n\n"
        f"В {bake_day} печём свежий хлеб на закваске.\n"
        f"Закажите сегодня до 22:00 — доставим завтра.\n\n"
        f"📋 Меню:\n{menu}\n\n"
        f"🎁 Промокод {PRE_BAKE_PROMO} — {PRE_BAKE_DISCOUNT} на весь заказ\n\n"
        f"Нажмите кнопку ниже, чтобы заказать 👇"
    )

def check_pre_bake_broadcast():
    """Проверяет, нужно ли отправить рассылку «Накануне выпечки».
    Вызывается из scheduler_loop каждую минуту."""
    now_msk = datetime.now(MSK_OFFSET)
    today_str = now_msk.strftime("%Y-%m-%d")

    # Уже отправляли сегодня?
    if pre_bake_last_sent.get("date") == today_str:
        return

    # Проверяем: сейчас >= PRE_BAKE_HOUR и завтра — день выпечки
    if now_msk.hour < PRE_BAKE_HOUR:
        return

    tomorrow_wd_js = ((now_msk.weekday() + 1) % 7 + 1) % 7  # перевод в JS-формат (0=вс)
    # BAKE_WEEKDAYS в формате JS (2=вт, 4=чт, 6=сб)
    if tomorrow_wd_js not in BAKE_WEEKDAYS:
        return

    # Время пришло — отправляем
    recipients = get_broadcast_recipients()
    if not recipients:
        print("Авто-рассылка: нет получателей")
        return

    text = build_pre_bake_message()
    keyboard = {
        "inline_keyboard": [
            [
                {"text": "🛒 Заказать", "url": f"https://t.me/{os.getenv('BOT_USERNAME', 'Sourdough_Bakery_Bot')}?start=order"},
                {"text": "📋 Меню", "url": f"https://t.me/{os.getenv('BOT_USERNAME', 'Sourdough_Bakery_Bot')}?start=menu"},
            ],
            [
                {"text": "🔕 Отписаться", "url": f"https://t.me/{os.getenv('BOT_USERNAME', 'Sourdough_Bakery_Bot')}?start=nobroadcast"},
            ],
        ]
    }
    ...
    for cid in recipients:
        try:
            payload = {"chat_id": cid, "text": text, "reply_markup": json.dumps(keyboard)}
            resp = requests.post(
            ...
            if resp.ok:
                ...
                if "blocked" in error_desc or "deactivated" in error_desc:
                    # Пользователь заблокировал бота — добавляем в optout
                    if cid not in broadcast_optout:
                        broadcast_optout.append(cid)
                        save_broadcast_optout()
                    blocked += 1
                else:
                    failed += 1
            ...
            failed += 1
            print(f"Авто-рассылка: ошибка отправки {cid}:", e)
        time.sleep(0.05)

    # Запоминаем дату
    with data_lock:
        pre_bake_last_sent["date"] = today_str
        save_pre_bake_last_sent()

    report = f"🔔 Авто-рассылка «Накануне выпечки» выполнена: отправлено={sent}, заблокировали={blocked}, ошибки={failed}"
    print(report)
    if ADMIN_CHAT_ID:
...
# ================= НАПОМИНАНИЕ О БРОШЕННОЙ КОРЗИНЕ =================
# Трекинг: {phone: {"reminded_at": ISO, "count": int}}
# reminded_at — когда последний раз напоминали; count — сколько раз (макс 2).
abandoned_cart_tracking = load_json("abandoned_cart_tracking.json", {})
def save_abandoned_cart_tracking():
    save_json("abandoned_cart_tracking.json", abandoned_cart_tracking)

def check_abandoned_carts():
    """Проверяет все непустые корзины и отправляет напоминание, если корзина
    не менялась дольше ABANDONED_CART_HOURS и заказ не был оформлен."""
    now = datetime.now(timezone.utc)

    for phone, cart_data in list(carts.items()):
        items = cart_data.get("items", {})
        if not items:
            # Корзина пуста — сбрасываем трекинг
            if phone in abandoned_cart_tracking:
                del abandoned_cart_tracking[phone]
                save_abandoned_cart_tracking()
            continue

        updated_at_str = cart_data.get("updatedAt")
        ...
        # Определяем, нужно ли напоминание
        tracking = abandoned_cart_tracking.get(phone, {"reminded_at": None, "count": 0})
        count = tracking.get("count", 0)

        # Первое напоминание
        if count == 0 and hours_since_update >= ABANDONED_CART_HOURS:
            should_remind = True
        # Второе напоминание (если включено)
        elif count == 1 and ABANDONED_CART_REPEAT_HOURS > 0:
            last_reminded = tracking.get("reminded_at")
            if last_reminded:
                try:
                    last_dt = datetime.fromisoformat(last_reminded)
                    if last_dt.tzinfo is None:
                        last_dt = last_dt.replace(tzinfo=timezone.utc)
                    hours_since_remind = (now - last_dt).total_seconds() / 3600
                    should_remind = hours_since_remind >= ABANDONED_CART_REPEAT_HOURS
                except (ValueError, TypeError):
                    should_remind = False
            else:
                should_remind = True
        ...
        # Проверяем, не оформлен ли заказ после обновления корзины
        recent_order = False
        for order in reversed(orders):
            if order.get("phone") == phone:
                ...
                try:
                    ...
                    if order_dt >= updated_at:
                        recent_order = True
                except (ValueError, TypeError):
                    pass
                break
        if recent_order:
            # Заказ оформлен — сбрасываем
            if phone in abandoned_cart_tracking:
                del abandoned_cart_tracking[phone]
                save_abandoned_cart_tracking()
            continue
        ...
        # Не напоминаем отписавшимся от рассылок
        if chat_id in broadcast_optout:
            continue
        ...
        if count == 0:
            ...
            text = (
                f"👋 У вас в корзине {item_count} шт. на {total} ₽\n\n"
                f"{cart_text(items)}\n\n"
                f"Оформить заказ? Нажмите кнопку ниже 👇"
            )
        else:
            # Второе — с промокодом, мотивация
            text = (
                f"🍞 Ваш хлеб всё ещё ждёт!\n\n"
                f"{cart_text(items)}\n\n"
                f"Промокод {PRE_BAKE_PROMO} — {PRE_BAKE_DISCOUNT} "
                f"на этот заказ. Действует до конца дня."
            )
        ...
        try:
            ...
            with data_lock:
                abandoned_cart_tracking[phone] = {
                    "reminded_at": now.isoformat(),
                    "count": count + 1,
                }
                save_abandoned_cart_tracking()
            print(f"Напоминание о корзине отправлено: {phone} (напоминание #{count + 1})")
        except Exception as e:
            print(f"Ошибка напоминания о корзине {phone}:", e)
...
def scheduler_loop():
    while True:
        ...
        try:
            run_scheduled_tasks()
            check_scheduled_broadcasts_sync()
            check_pre_bake_broadcast()
...
# Создание онлайн-платежа — вызывается виджетом при выборе «Картой онлайн» или «СБП».
# Виджет передаёт orderId и total, сервер создаёт платёж через настроенный провайдер
# и возвращает ссылку — ключи остаются на сервере, клиент их не видит.
@flask_app.route("/api/create-payment", methods=["POST"])
def create_payment_route():
    ...
    if not order_id or not total:
        return jsonify({"error": "Не указан orderId или total"}), 400
    if not online_payment_configured():
        return jsonify({"error": f"Провайдер {PAYMENT_PROVIDER} не настроен"}), 503
    try:
        fake_order = {"id": order_id, "total": float(total), "phone": phone}
        result = create_online_payment(fake_order)
        return jsonify({"confirmationUrl": result["confirmation_url"]})
    except Exception as e:
        print(f"Ошибка /api/create-payment ({PAYMENT_PROVIDER}):", e)
...
def main():
    ...
    application.add_handler(CommandHandler("nobroadcast", nobroadcast_cmd))
    application.add_handler(CommandHandler("yesbroadcast", yesbroadcast_cmd))
