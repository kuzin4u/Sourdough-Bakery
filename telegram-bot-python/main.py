...
# ================= ОНЛАЙН-ОПЛАТА (ЮKassa, опционально) =================
...
def payment_keyboard():
    ...
    label = "Картой онлайн" if yookassa_configured() else "Картой онлайн (пока недоступно)"
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if data.startswith("pay:"):
            method = data.split(":")[1]
            if method == "online" and not yookassa_configured():
                await query.answer("Онлайн-оплата временно недоступна — выберите другой способ", show_alert=True)
                return
            session["draft"]["payment"] = "Наличными" if method == "cash" else "Картой при получении" if method == "card" else "Картой онлайн (ЮKassa)"
        ...
        if data == "confirm":
            ...
            if order["payment"].startswith("Картой онлайн"):
                try:
                    payment = await asyncio.to_thread(create_yookassa_payment, order)
    ...
    try:
        ...
        if event.get("event") == "payment.succeeded":
            ...
            if order:
                ...
                if ADMIN_CHAT_ID:
                    send_telegram_message(ADMIN_CHAT_ID, f"💳 Оплата по заказу №{order_id} прошла успешно.")
            else:
                print("Webhook ЮKassa: не нашли заказ по orderId из metadata:", order_id)
        return "", 200
    except Exception as e:
