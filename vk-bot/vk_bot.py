"""
VK-бот «Масса Матере» — зеркало Telegram-бота
===============================================
Та же логика: каталог из Google Sheets, корзина в carts.json,
заказы в Google Sheets через Apps Script webhook.

Настройка:
  VK_TOKEN       — ключ доступа сообщества (Управление → API)
  VK_GROUP_ID    — ID сообщества (число)
  VK_SECRET      — секрет Callback API (из настроек сервера)
  SHEETS_WEBHOOK_URL — тот же URL Apps Script что у Telegram-бота
  BAKE_WEEKDAYS  — дни выпечки (2,4,6 = Вт,Чт,Сб)

Деплой: отдельный Render Web Service, порт из env PORT.
"""
...
ADMIN_CHAT_ID    = os.getenv("VK_ADMIN_ID", "")  # VK user_id администратора
...
def load_products_from_sheets():
    ...
    try:
        ...
        if isinstance(data, list) and data:
            ...
            logger.info(f"VK-бот: загружено {len(PRODUCTS)} товаров из Sheets")
    except Exception as e:
        logger.warning(f"Не удалось загрузить товары VK: {e}")
...
def vk_api(method, **params):
    ...
    try:
        ...
        logger.error(f"VK API {method} error: {e}")
        return None
...
def next_bake_date():
    ...
    # BAKE_WEEKDAYS в JS-формате (0=вс,1=пн,...), Python: 0=пн,...,6=вс
    # Конвертируем: JS 2=вт → Python 1=вт
    py_bake = [(d - 1) % 7 for d in BAKE_WEEKDAYS]
    for i in range(1, 8):
        d = now + timedelta(days=i)
        if d.weekday() in py_bake:
...
def create_order(uid, name, phone, address):
    ...
    for pid, qty in items.items():
        p = next((x for x in PRODUCTS if x["id"] == pid), None)
        if p:
            items_list.append({"name": p["name"], "qty": qty, "subtotal": p["price"] * qty})
    order = {
        "id": order_id,
        "channel": "VKontakte",
        "createdAt": datetime.now(timezone.utc).isoformat(),
        "status": "Принят",
        "bakeDate": next_bake_date(),
        "name": name,
        "phone": phone,
        "address": address,
        "items": items_list,
...
def handle_message(user_id, text, payload):
    ...
    if cmd == "add" and pid:
        ...
        p = next((x for x in PRODUCTS if x["id"] == pid), None)
        if p:
            send_message(user_id, f"✅ {p['name']} добавлен в корзину!\n\nВсего в корзине: {sum(items.values())} шт на {cart_total(items)} ₽", kb_catalog())
        return
    ...
    if cmd == "cart" or text.lower() in ["корзина", "заказ"]:
        ...
        if not items:
            ...
            send_message(user_id, f"🧺 Ваша корзина:\n\n{cart_text(items)}\n\n💰 Итого: {cart_total(items)} ₽", kb_cart(uid))
        return
    ...
    if cmd == "orders" or text.lower() in ["заказы", "мои заказы"]:
        my_orders = [o for o in orders if str(o.get("chatId")) == uid or str(o.get("phone","")).endswith(uid[-4:])][-3:]
        if not my_orders:
            send_message(user_id, "У вас пока нет заказов.", kb_main())
        else:
            text_out = "📦 Ваши заказы:\n\n"
            for o in reversed(my_orders):
                text_out += f"#{o['id']} — {o['status']}\n{o.get('bakeDate','?')} · {o['total']} ₽\n\n"
            send_message(user_id, text_out, kb_main())
        return

    # НАЗАД / СТАРТ
    if cmd in ("back", "start") or text.lower() in ["/start", "старт", "начало", "привет"]:
        session["stage"] = "idle"
        send_message(user_id,
            "👋 Привет! Я бот пекарни Масса Матере 🍞\n\n"
            "Свежий хлеб на закваске — выпечка вт/чт/сб.\n"
    ...
    if session["stage"] == "awaiting_address":
        ...
        order = create_order(uid, name, phone, text)
        if order:
            summary = cart_text({i["name"]: i["qty"] for i in order["items"]})
            send_message(user_id,
                ...
                "Спасибо за заказ! 🙏", kb_main())
            notify_admin(
                f"🔔 Новый заказ VK #{order['id']}\n"
                f"👤 {name} · {phone}\n"
                f"📍 {text}\n"
                f"💰 {order['total']} ₽ · {order['bakeDate']}"
            )
        return

    # По умолчанию
    send_message(user_id,
        "Привет! Используйте кнопки меню ниже 👇", kb_main())
...
if __name__ == "__main__":
    ...
    logger.info(f"VK-бот запущен на порту {PORT}")
