...
# По умолчанию = тот же URL что и REVIEWS_API_URL (единый эндпоинт sheets-unified-api.gs)
ORDERS_API_URL="${ORDERS_API_URL:-$REVIEWS_API_URL}"
if [ -n "$ORDERS_API_URL" ]; then
  echo "ORDERS_API_URL=$ORDERS_API_URL"
  sed -i "s|__ORDERS_API_URL__|${ORDERS_API_URL}|g" cabinet-orders.html 2>/dev/null || true
  sed -i "s|__ORDERS_API_URL__|${ORDERS_API_URL}|g" cabinet-analytics.html 2>/dev/null || true
else
